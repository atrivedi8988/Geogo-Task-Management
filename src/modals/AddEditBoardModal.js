...
import { addBoards, updateBoards } from "../redux/boardsSlice";
...
function AddEditBoardModal({ setIsBoardModalOpen, type, }) {
  ...
  const onSubmit = (type) => {
    ...
    if (type === "add") {
      dispatch(addBoards({ name, columns: newColumns, "isActive": false, }));
    } else {
      dispatch(updateBoards({ name, columns: newColumns, "isActive": false, id: board?.id }));
    }
  };
